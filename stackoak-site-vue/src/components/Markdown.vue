...
import {ref, onMounted, onUnmounted, watch, onActivated, onUpdated, reactive} from "vue";
import "cherry-markdown/dist/cherry-markdown.min.css";
import Cherry from "cherry-markdown";

const emit = defineEmits(['markdown-change'])
const props = defineProps(['mdId', 'height', 'width', 'preview', 'value'])
let cherryInstance = reactive<Cherry | null>(null);
onMounted(() => {
  initCherryMD()
});

onUnmounted(async () => {
  await cherryInstance.destroy()
})
...
const callbacks = {
  ...
  afterChange: (text: any, html: any) => {
    try {
      console.log(cherryInstance.getValue())
      emit("markdown-change", {content: text, toc: cherryInstance.getToc()});
    } catch (error) {
...
const initCherryMD = () => {
  const defaultConfig = {
    ...
    engine: {
      ...
      syntax: {
        ...
        codeBlock: {
         // theme: 'coy', //  @deprecated 不再支持theme的配置，统一在`themeSettings.codeBlockTheme`中配置
    ...
    previewer: {
      ...
      enablePreviewerBubble: props.preview ? true : false,
  ...
  cherryInstance = new Cherry(defaultConfig)
}
</script>
<style scoped>
:deep(.cherry) {
