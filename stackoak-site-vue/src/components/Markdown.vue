...
const callbacks = {
  ...
  afterChange: (text: any, html: any) => {
    try {
      console.log(cherryInstance.value.getValue())
      emit("markdown-change", {content: text, toc: cherryInstance.value.getToc()});
    } catch (error) {
...
const initCherryMD = () => {
  ...
  cherryInstance.value = new Cherry(defaultConfig)
}
</script>
<style scoped>
:deep(.cherry) {
