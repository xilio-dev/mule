...
const downloadChange = async () => {
  ...
  a.download = `${articleInfo.value.id}.png`;
  a.href = url;
    ...
    <a-popover placement="left">
      <template #content>
        <a-flex vertical align="center">
          <a-qrcode ref="qrcodeCanvasRef" :value="`http://192.168.0.151:3002/post/${articleInfo.id}`"/>
          <a-flex gap="8" justify="flex-end" align="center">
            <span>分享二维码</span>
            <span style="color: #1e80ff;cursor: pointer" @click="downloadChange">下载</span>
          </a-flex>
        </a-flex>
      </template>
